...
import { Waves, CheckCircle, Users, Phone, Calendar, Droplets } from 'lucide-react';
...
export default function PoolStaffDashboard() {
    const [bookings, setBookings] = useState<PoolBooking[]>([]);
    ...
    const fetchBookings = useCallback(async () => {
        try {
            ...
            if (res.ok) {
                const data: PoolBooking[] = await res.json();

                const sorted = data.sort((a, b) => (a.booking_date || '').localeCompare(b.booking_date || ''));
                setBookings(sorted);

                const todayString = new Date().toISOString().split('T')[0];
                const todayBookings = data.filter((b) => b.booking_date === todayString && b.status !== 'cancelled');

                setStats({
                    visitors: todayBookings.reduce((acc, b) => acc + (b.adults || 0) + (b.children || 0), 0),
                    revenue: todayBookings.reduce((acc, b) => acc + (b.total_price || 0), 0),
                    active: todayBookings.filter((b) => b.status === 'checked_in').length
                });
            }
        } catch (err) {
    ...
    const updateStatus = async (id: string, status: string) => {
        const update: Partial<PoolBooking> & { type: string, checked_in_at?: string, completed_at?: string } = { status, type: 'pool' };
        if (status === 'checked_in') update.checked_in_at = new Date().toISOString();
        if (status === 'completed') update.completed_at = new Date().toISOString();

        setBookings(prev => prev.map(b => b.id === id ? { ...b, ...update } as PoolBooking : b));
    ...
    const getFilteredBookings = () => {
        ...
        return bookings.filter(b => {
            if (filter === 'today') return b.booking_date === todayString;
            if (filter === 'upcoming') return b.booking_date > todayString;
            return true;
        });
    };
...
function KPICard({ icon, label, value, color, bg }: { icon: ReactNode, label: string, value: number, color: string, bg: string }) {
    return (
